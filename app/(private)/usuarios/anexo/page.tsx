...
export default function UsuariosPage() {
  ...
  async function generate(type:"Contrato comercial"|"Anexo de comisiones"){
    if(!editing||!profile?.id){
      alert("Primero guarda la ficha comercial.");
      return;
    }
    ...
    try{
      if(type==="Contrato comercial"){
        const r=await fetch("/api/commercial-documents",{
          method:"POST",
          headers:{"Content-Type":"application/json"},
          body:JSON.stringify({
            userId:editing.id,
            commercialProfileId:profile.id,
            documentType:"Contrato comercial",
            profileSnapshot:persistedProfileType||null
          })
        });
        ...
        if(!r.ok||!d.ok){
          alert(d.error||"No se pudo generar el contrato.");
          return;
        }

        const contract=d.document;

        // El anexo económico inicial se genera inmediatamente con las
        // condiciones existentes en este mismo momento.
        const ar=await fetch("/api/commercial-annex",{
          method:"POST",
          headers:{"Content-Type":"application/json"},
          body:JSON.stringify({
            userId:editing.id,
        ...
        if(!ar.ok||!ad.ok){
          // El contrato existe y no se borra. Informamos claramente de que
          // el anexo no pudo emitirse para corregir el catálogo económico.
          setDocuments(v=>[contract,...v]);
          alert(
            `Contrato creado correctamente, pero no se pudo generar el anexo económico: ${ad.error||"revisa las comisiones del perfil."}`
          );
          return;
        }

        setDocuments(v=>[ad.document,contract,...v]);

        alert(
          `Contrato y anexo económico creados correctamente.\n\nEl anexo ha congelado las comisiones vigentes del perfil ${persistedProfileType||editing.profile_type||""}.`
        );
        return;
      }

      // Generación manual de anexo inicial solo para casos excepcionales
      // o contratos históricos que no dispongan de él.
      const r=await fetch("/api/commercial-annex",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          userId:editing.id,
      ...
      if(!r.ok||!d.ok){
        alert(d.error||"No se pudo generar el anexo económico.");
        return;
      }
      ...
      alert(e?.message||"No se pudo generar la documentación.");
    }finally{
      setSaving(false);
    }
  }
  ...
  async function updateDocumentStatus(doc: CommercialDocument, status: "Firmado"|"Sustituido"|"Cancelado"){
    const visibleName =
      doc.document_type==="Anexo de comisiones"
        ? "Anexo de condiciones económicas"
        : `${doc.title} v${doc.version}`;

    const question =
      status==="Firmado" ? `¿Marcar "${visibleName}" como firmado?` :
      status==="Sustituido" ? `¿Marcar "${visibleName}" como sustituido?` :
      `¿Cancelar "${visibleName}"?`;
      ...
      <section style={modal} onClick={e=>e.stopPropagation()}>
        ...
        {tab==="documentos"&&<div style={{marginTop:18}}>
          <div style={{border:"1px solid #ffd7c7",background:"#fff7f2",borderRadius:14,padding:14}}>
            ...
            <p style={{margin:"4px 0 0",fontSize:12,color:"#6e5a52"}}>Al generar el contrato, ONE crea automáticamente el anexo económico con las comisiones vigentes en ese mismo momento.</p>
            ...
            <div style={{display:"flex",gap:10,marginTop:12,flexWrap:"wrap"}}>
              <button style={primary} onClick={()=>generate("Contrato comercial")}>Generar contrato + anexo económico</button>
              <button style={secondary} onClick={()=>generate("Anexo de comisiones")}>Crear anexo para contrato histórico</button>
            </div>
          </div>
          <h3>Histórico</h3>
            ...
            documents.map(d=><div key={d.id} style={{
              ...
              <div>
                <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                  <b style={{fontSize:13}}>
                    {d.document_type==="Anexo de comisiones"?"Anexo de condiciones económicas":d.title}
                  </b>
                  {d.document_type!=="Anexo de comisiones"&&
              ...
              <div style={{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>
                <button style={secondary} onClick={()=>openDocument(d)}>
                  {d.document_type==="Anexo de comisiones"?"Ver anexo económico":"Ver contrato"}
                </button>
