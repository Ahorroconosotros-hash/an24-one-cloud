...
export default function ClienteForm({
  ...
  useEffect(() => {
    const name =
      ...
      "Usuario actual";
    const role = (
      window.localStorage.getItem("one_current_user_role") ||
      window.localStorage.getItem("one_user_role") ||
      "Administrador"
    ).toLowerCase();

    setCurrentUser(name);
    setCanAssignCommercial(role.includes("admin") || role.includes("backoffice"));

    if (mode === "create" && !draft.commercial) {
      setDraft((current) => ({ ...current, commercial: name }));
    }
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const result =
      mode === "edit" && initial
        ? updateClient(initial.id, normalizedDraft)
        : createClient(normalizedDraft);

    if (!result) {
      setMessage("No se ha podido guardar el cliente.");
      setSaving(false);
      return;
    }
        ...
        <div className={styles.grid3}>
          ...
          <div className={`${styles.assignment} ${styles.span2}`}>
            <div>
              ...
              <strong>{draft.commercial || currentUser}</strong>
              <small>Al crear la ficha se asigna automáticamente al usuario que la registra.</small>
            </div>
            {mode === "edit" && canAssignCommercial ? (
              <label className={styles.field}>
                <span>Gestionado por BackOffice / Administración</span>
                <input
                  value={draft.commercial}
                  onChange={(event) => set("commercial", event.target.value)}
                  placeholder="Asignar o corregir comercial"
                />
              </label>
            ) : (
              <span className={styles.locked}>🔒 Solo BackOffice o Administración puede cambiarlo</span>
            )}
          </div>
        </div>
      </section>
